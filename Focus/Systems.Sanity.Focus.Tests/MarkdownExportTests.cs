...
public class MarkdownExportTests
{
    ...
    public void Export_MarkdownWithAttachments_RendersRootQuotesAndChildImageLinks()
    {
        ...
        var expectedImagePath = $"_attachments/{GetRequiredNodeIdentifier(child):D}/{childImageAttachment.RelativePath}".ToLowerInvariant();
    ...
    [Fact]
    public void Export_MarkdownUnderChildShowDoneOverride_RestoresDoneDescendants()
    {
        var map = new MindMap("Root");
        ...
        var markdown = MapExportService.Export(branch, ExportFormat.Markdown);

        Assert.Contains("# Branch", markdown);
        Assert.Contains("[ ] Todo child", markdown);
        Assert.Contains("[x] Done child", markdown);
    }
    ...
    public void Export_MarkdownWithMissingAttachment_FallsBackToRelativeLink()
    {
        ...
        var markdown = MapExportService.Export(
            ...
            new NodeExportOptions(
                ...
                MapFilePath: filePath,
                ExportFilePath: exportFilePath));
        var expectedMissingPath = $"_attachments/{GetRequiredNodeIdentifier(map.RootNode):D}/missing.txt".ToLowerInvariant();

        Assert.Contains(
            $"[Missing clipboard text.txt](<{expectedMissingPath}>)",
            markdown);
